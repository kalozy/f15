...
class WhatsAppBot {
  constructor(io, dataDir, userId) {
    this.io = io;
    this.userId = userId || null;   // for scoped socket events
  ...
  initAI() {
    // User-specific key takes priority over env
    const saved = (() => { try { return JSON.parse(fs.readFileSync(this.FLOWS_PATH,'utf8')); } catch { return {}; } })();
    const key     = saved.anthropicApiKey || process.env.ANTHROPIC_API_KEY;
    const groqKey = saved.groqApiKey      || process.env.GROQ_API_KEY;
  ...
  setStatus(status, extra = {}) {
    this.status = status;
    // Emit only to this user's socket room (multi-tenant isolation)
    const target = (this.userId && this.io.to) ? this.io.to('user:' + this.userId) : this.io;
    target.emit('bot:status', { status, ...extra });
    const _statusEmoji = {connected:'✓',connecting:'…',disconnected:'✗',qr:'📱'};
    console.log('[BOT] user:' + (this.userId||'?') + ' ' + (_statusEmoji[status]||'?') + ' ' + status);
  }
